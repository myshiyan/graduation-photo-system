    ...
    db.all('SELECT * FROM photos WHERE class_id = ?', [req.params.id], (err, photos) => {
        ...
        const deletePromises = photos.map(photo => {
            return new Promise((resolve, reject) => {
                db.run('DELETE FROM names WHERE photo_id = ?', [photo.id], (err) => {
                    if (err) reject(err);
                    else {
                        db.run('DELETE FROM faces WHERE photo_id = ?', [photo.id], (err) => {
                            if (err) reject(err);
                            else {
...
app.get('/api/photos/:classId', (req, res) => {
    db.get('SELECT * FROM photos WHERE class_id = ?', [req.params.classId], (err, photo) => {
        ...
        db.all('SELECT * FROM names WHERE photo_id = ? ORDER BY row_index, name_index', [photo.id], (err, names) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            db.all('SELECT * FROM faces WHERE photo_id = ? ORDER BY face_index', [photo.id], (err, faces) => {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }
...
app.post('/api/photos', upload.single('photo'), (req, res) => {
    ...
    db.get('SELECT * FROM classes WHERE id = ?', [classId], (err, cls) => {
        ...
        db.get('SELECT * FROM photos WHERE class_id = ?', [classId], (err, existingPhoto) => {
            ...
            const insertPhoto = () => {
                db.run('INSERT INTO photos (class_id, file_path, filename, title, date) VALUES (?, ?, ?, ?, ?)',
                    [classId, filePath, req.file.originalname, title, date], function(err) {
                        if (err) {
                            return res.status(500).json({ error: err.message });
                        }
                        res.json({ success: true, id: this.lastID });
            ...
            if (existingPhoto) {
                ...
                db.run('DELETE FROM names WHERE photo_id = ?', [existingPhoto.id], (err) => {
                    if (err) return res.status(500).json({ error: err.message });
                    db.run('DELETE FROM faces WHERE photo_id = ?', [existingPhoto.id], (err) => {
                        if (err) return res.status(500).json({ error: err.message });
...
function deletePhotoById(photoId, res) {
    db.get('SELECT * FROM photos WHERE id = ?', [photoId], (err, photo) => {
        ...
        db.run('DELETE FROM names WHERE photo_id = ?', [photoId], (err) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
...
app.post('/api/names', (req, res) => {
    ...
    const { photoId, classId, title, date, names } = req.body;

    if (!photoId) {
        return res.status(400).json({ error: '缺少photoId' });
    }
    ...
    db.run('UPDATE photos SET title = ?, date = ? WHERE id = ?', [title, date, photoId], (err) => {
        ...
        db.run('DELETE FROM names WHERE photo_id = ?', [photoId], (err) => {
            ...
            const insertPromises = names.map(n => {
                return new Promise((resolve, reject) => {
                    db.run('INSERT INTO names (photo_id, row_index, name_index, name) VALUES (?, ?, ?, ?)',
                        [photoId, n.row_index, n.name_index, n.name], (err) => {
                            if (err) reject(err);
                            else resolve();
                        });
                });
...
app.put('/api/names', (req, res) => {
    ...
    const photoId = names[0].photo_id;
    ...
    db.run('DELETE FROM names WHERE photo_id = ?', [photoId], (err) => {
        ...
        const insertPromises = names.map(n => {
            return new Promise((resolve, reject) => {
                db.run('INSERT INTO names (photo_id, row_index, name_index, name) VALUES (?, ?, ?, ?)',
                    [n.photo_id, n.row_index, n.name_index, n.name], (err) => {
                        if (err) reject(err);
                        else resolve();
                    });
            });
...
app.post('/api/photos/:classId', upload.single('photo'), (req, res) => {
    ...
    db.get('SELECT * FROM classes WHERE id = ?', [classId], (err, cls) => {
        ...
        db.get('SELECT * FROM photos WHERE class_id = ?', [classId], (err, existingPhoto) => {
            ...
            const insertPhoto = () => {
                const filePath = req.file.path.replace(__dirname + '\\public\\', '').replace(__dirname + '/public/', '');
                db.run('INSERT INTO photos (class_id, file_path, filename) VALUES (?, ?, ?)',
                    [classId, filePath, originalFilename], (err) => {
                        if (err) {
                            return res.status(500).json({ error: err.message });
                        }
                        res.json({ success: true });
            ...
            db.get('SELECT * FROM classes WHERE name LIKE ?', [`%${className}%`], (err, cls) => {
                ...
                db.get('SELECT * FROM photos WHERE class_id = ?', [cls.id], (err, existingPhoto) => {
                    ...
                    const insertPhoto = () => {
                        ...
                        db.run('INSERT INTO photos (class_id, file_path, filename) VALUES (?, ?, ?)',
                            [cls.id, filePath, file.originalname], (err) => {
                                if (err) {
                                    failedCount++;
                                } else {
                                    successCount++;
